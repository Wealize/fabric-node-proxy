...
async function main() {
    try {
        ...
        const enrollment = await ca.enroll({
            enrollmentID: userUsername, enrollmentSecret: userPassword })
        const userIdentity = X509WalletMixin.createIdentity(
            "Org1MSP", enrollment.certificate, enrollment.key.toBytes())
        await wallet.import(userUsername, userIdentity)
